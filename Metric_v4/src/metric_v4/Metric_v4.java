...
public static void main(String[] args) {
    ...
    boolean BranchLoopFor = false;
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(file))) {
            ...
            while (fw.next()) {
                ...
                if (pe instanceof LoopStatement || pe instanceof BranchStatement) {
                    ...
                    if(!isTryFor.equals("Try") && !isTryFor.equals("Do")){
                        ...
                        String expr = Metric_v4.expression(pe.getFirstElement().toSource(), aux);
                        
                        if(!expr.startsWith("ERROR ")){
                            
                            bw.write("\t\t\texpression " + expr + "\n");
                            int numCond = Metric_v4.contCondition(expr);
                            String rep = "";
                            if(Metric_v4.equalNumberCondition(expr) == 1){
                                rep = "Yes Rep";
                            } else {
                                rep = "No Rep";
                            }
                            Metric_v4.recorMark(numCond);
            ...
            Logger.getLogger(Metric_v4.class.getName()).log(Level.SEVERE, null, ex);
        }   
  ...
  public static void recorMark(int val){
      ...
      if(Metric_v4.mapCon.containsKey(String.valueOf(val))) {
          
          aux = Integer.parseInt(Metric_v4.mapCon.get(String.valueOf(val)));
          aux ++;
          
          Metric_v4.mapCon.remove(String.valueOf(val));
          Metric_v4.mapCon.put(String.valueOf(val), String.valueOf(aux));
          
      } else {
          
          Metric_v4.mapCon.put(String.valueOf(val), String.valueOf(1));
   ...
   public static void printHash(){
       ...
       Set<String> setKey = Metric_v4.mapCon.keySet();
       ...
       DecimalFormat df = new DecimalFormat("0.00"); 
           ...
           try (BufferedWriter bw = new BufferedWriter(new FileWriter(file))) {
               for (String string : setKey) {
                   ...
                   numOpA = (double)Integer.parseInt(Metric_v4.mapCon.get(string));
                   numOpB = (double)ListMetric.statementLoopBranch;
                   num = ((numOpA/numOpB) * 100.0);
                   bw.write("Number of Conditions " + string + " =  $ " + Metric_v4.mapCon.get(string) + "  $ " + df.format(num) + "%"  + "\n");
